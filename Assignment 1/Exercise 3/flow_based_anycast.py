...
class L3Switch(app_manager.RyuApp):
    ...
    def select_server_for_flow(self, ipv4_pkt, src_port, dst_port):
        known_servers = [
            server for server in self.ANYCAST_GROUP.servers
            if server.mac is not None
        ]

        if len(known_servers) < len(self.ANYCAST_GROUP.servers):
            return None
        ...
        key_string = "%s:%s-%s:%s-%s" % flow_key
        flow_hash = int(hashlib.sha256(key_string.encode()).hexdigest(), 16)

        return known_servers[flow_hash % len(known_servers)]
    ...
    def do_anycast(self, datapath, in_port, msg, pkt, ipv4_pkt):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
        ...
        if tcp_pkt is None:
            self.logger.info("flow-anycast: ignoring non-TCP packet")
            return

        src_port = tcp_pkt.src_port
        dst_port = tcp_pkt.dst_port

        server = self.select_server_for_flow(ipv4_pkt, src_port, dst_port)
        ...
        self.logger.info(
            "flow-anycast: %s:%s -> %s:%s mapped to server %s via port %d",
            ipv4_pkt.src,
            src_port,
            ipv4_pkt.dst,
            dst_port,
        ...
        match = parser.OFPMatch(
            ...
            eth_dst=self.ANYCAST_GROUP.anycast_mac,
    ...
    def _packet_in_handler(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_IP:
            ...
            if dst_ip == self.ANYCAST_GROUP.anycast_ip and dst_mac == self.ANYCAST_GROUP.anycast_mac:
                self.do_anycast(datapath, in_port, msg, pkt, ipv4_pkt)
            else:
